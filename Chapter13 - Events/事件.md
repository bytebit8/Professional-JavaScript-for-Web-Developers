...
由于鼠标事件可以结合键盘事件一起使用，因此在多事件组合使用的场景下获取键盘上某些特殊按键的状态显得非常有用，例如按住 `ctrl` 键单击鼠标的操作，此时这里的 `ctrl` 键便是修饰键。

鼠标事件的事件对象中，类似的修饰键还有 `shift` ，`alt`，`window(mac 为command)` 等，下面是它们一一对象的事件对象属性：
