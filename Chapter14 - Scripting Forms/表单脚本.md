...
在 HTML 中“表单”表示的 `<form>` 标记，而在 JavaScript 中“表单”表示的是一个 `HTMLFormElement` 类型的 DOM 元素节点对象。
...
设置剪贴板中的数据，相同的 `format` 参数在IE中只支持 `text` 与 `URL` 。而现代浏览器中只支持 MIME 类型并不能像 `getData()` 方法那样还可以识别 `text` 类型。
...
>
> 4. `<select>` 元素的值是选中项 `<option>` 的 `value` 值，如果选中项没有 `value` 属性，则去该选项的文本值 `text` 。
>
